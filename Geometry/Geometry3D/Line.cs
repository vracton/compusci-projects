...
namespace Geometry.Geometry3D
{
    ...
    public class Line
    {
        ...
        {
            // Defining the two lines as p1 + t1 * v1 and p2 + t2 * v2, with v1 and v2 unit vectors,
            // We require the dot product with both lines to be zero at the values of t1 and t2 we want.
            // Solving gives t1 = (p2 - p1) dot ((v1 - d v2) / (1 - d^2)), where d = v1 dot v2
            Vector unit = UnderlyingRay.Direction.UnitVector();
            Vector otherUnit = line.UnderlyingRay.Direction.UnitVector();
            double dot = Vector.Dot(unit, otherUnit);
            if (dot == 1)
            {
                // This is parallel lines
                // Then any point will work
                return line.UnderlyingRay.EndPoint;
            }
            Vector pDiff = line.UnderlyingRay.EndPoint - UnderlyingRay.EndPoint;
            Vector otherV = (unit - dot * otherUnit) / (1 - dot * dot);
            double parameter = Vector.Dot(pDiff, otherV);
            return UnderlyingRay.EndPoint + parameter * unit;
        }
        ...
        public Point NearestPoint(LineSegment segment)
        {
            ...
            return point; // The closest point on the line is also the closest to the segment
