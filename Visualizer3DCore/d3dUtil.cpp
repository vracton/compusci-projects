...
ComPtr<ID3DBlob> D3DUtil::loadBinary(const std::wstring& filename)
{
    ...
    D3DUtil::throwIfFailed(D3DCreateBlob(size, blob.GetAddressOf()));
    ...
    Microsoft::WRL::ComPtr<ID3D12Resource>& uploadBuffer)
{
    ...
    D3DUtil::throwIfFailed(device->CreateCommittedResource(
        &heapProperties,                                // heap propoertie
		...
		IID_PPV_ARGS(defaultBuffer.GetAddressOf())));   // resource
    ...
    D3DUtil::throwIfFailed(device->CreateCommittedResource(
		...
		nullptr,                                        // optimized clear value
		IID_PPV_ARGS(uploadBuffer.GetAddressOf())));    // resource


    // Describe the data we want to copy into the default buffer.
    D3D12_SUBRESOURCE_DATA subResourceData = {
		initData,                          // pointer to the initialization data
		static_cast<LONG_PTR>(byteSize),   // size of all the data for one row
        static_cast<LONG_PTR>(byteSize)    // size of all the data for one depth slice
    };
